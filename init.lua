...
local sailstep = function(self)
	...
	if sailrot then
		...
		local longit_drag =
			vct.multiply(hdir, longit_speed * longit_speed * LONGIT_DRAG_FACTOR * -1 * sign(longit_speed))
		...
		if self.driver then
			...
			if plyr then
				...
				if self.sail_set then
					...
					if paddleacc then
						accel = vct.add(accel, vct.multiply(hdir, paddleacc))
					end
				end
		...
		if abs(newroll - roll) > ROLL_RATE * self.dtime then
			newroll = roll + ROLL_RATE * self.dtime * sign(newroll - roll)
		end
		if newroll ~= roll or newyaw ~= yaw or newpitch ~= pitch then
			self.object:set_rotation({ x = newpitch, y = newyaw, z = newroll })
		end

		-- workaround for broken attachments
		if random() > 0.95 then
			self.sail:set_attach(self.mast, "", { x = 0, y = 0, z = 0 }, { x = 0, y = 0, z = 0 })
		end
	end
end
